...
function KompaniaLogIn() {
    const [email,setEmail] = useState()
    const [password,setPassword] = useState()
    const [user, setUser] = useState([]);
    const [userSet, setUserSet] = useState(localStorage.getItem('User'))
    useEffect(()=>{
    ...
    const logIn = () =>{
        axios.post("https://localhost:7235/login/ipko/"+email+"/"+password).then(response=>{
          const newUser = {authToken: response.data};
          setUser([newUser])
            localStorage.setItem("control",JSON.stringify([newUser]))
            window.location.href='http://localhost:3000/dashboard/internet' 
        })
    }
    return (
      <>
